...
`),o.markdown=i[0]&&typeof i[0].getAttribute(`data-markdown`)==`string`),t.postMessage(JSON.stringify(o),`*`)}function c(e){try{return window.location.origin===e.source.location.origin}catch{return!1}}function l(t){if(c(t))try{let n=JSON.parse(t.data);n&&n.namespace===`reveal-notes`&&n.type===`connected`?(clearInterval(e),u()):n&&n.namespace===`reveal-notes`&&n.type===`call`&&o(n.methodName,n.arguments,n.callId)}catch{}}function u(){n.on(`slidechanged`,s),n.on(`fragmentshown`,s),n.on(`fragmenthidden`,s),n.on(`overviewhidden`,s),n.on(`overviewshown`,s),n.on(`paused`,s),n.on(`resumed`,s),n.on(`previewiframe`,s),n.on(`previewimage`,s),n.on(`previewvideo`,s),n.on(`closeoverlay`,s),s()}return{id:`notes`,init:function(e){n=e,/receiver/i.test(window.location.search)||(window.location.search.match(/(\?|\&)notes/gi)===null?window.addEventListener(`message`,e=>{if(!t&&typeof e.data==`string`){let t;try{t=JSON.parse(e.data)}catch{}t&&t.namespace===`reveal-notes`&&t.type===`heartbeat`&&i(e.source)}}):r(),n.addKeyBinding({keyCode:83,key:`S`,description:`Speaker notes view`},function(){r()}))},open:r}},Kt=document.querySelector(`#demo-frame`),qt=document.querySelector(`#reload-demo`),Jt=`/fixing-vertec-with-ai/`;function Yt(e){return`${Jt}${e.replace(/^\/+/,``)}`}function Xt(){document.getElementById(`deck-chrome-reset`)?.remove();let e=document.createElement(`style`);e.id=`deck-chrome-reset`,e.textContent=`
  ...
  `,document.head.append(e)}function Zt(){let e=Kt?.contentDocument;if(!e||e.querySelector(`[data-vertec-helper-v1-loader]`))return;let t=e.createElement(`script`);t.src=Yt(`prototypes/v1/vertec-helper.user.js`),t.dataset.vertecHelperV1Loader=`true`,e.head.append(t)}var Qt=new F({hash:!0,controls:!1,progress:!1,slideNumber:!1,width:1600,height:900,margin:.035,center:!1,transition:`none`,backgroundTransition:`none`,plugins:[Gt]});Xt(),Qt.initialize(),(!window.location.hash||window.location.hash===`#/3`||window.location.hash===`#/8`)&&(window.history.replaceState(null,``,`#/6`),Qt.slide(6)),Kt?.addEventListener(`load`,Zt),Kt?.contentDocument?.readyState&&Kt.contentDocument.readyState!==`loading`&&Zt(),qt?.addEventListener(`click`,()=>{if(Kt){let e=new URL(Yt(`fixtures/vertec-workshop-copy.html`),window.location.href);e.searchParams.set(`reload`,String(Date.now())),Kt.src=e.toString()}});